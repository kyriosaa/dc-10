# DC-10 LED Chaser Board

[![Electronics](https://img.shields.io/badge/Electronics-B03931?style=flat&logo=data:image/svg+xml;base64,&logoColor=white)]()
[![PCB](https://img.shields.io/badge/PCB-008C4A?style=flat&logo=data:image/svg+xml;base64,&logoColor=white)]()
[![Altium](https://img.shields.io/badge/Altium_Designer-A5915F?style=flat&logo=altiumdesigner&logoColor=white)](https://www.altium.com/)

A 10-LED chaser that sequentially illuminates LEDs using a CD74HC4017E decade counter with adjustable speed control.

## Features

- **10-LED sequential display** - Uses a decade counter (CD74HC4017E) to drive 10 LEDs in a sequence
- **Adjustable chase speed** - Variable timing control via potentiometer for LED sequence speed
- **Auto-reset function** - Automatically cycles through all 10 outputs continuously
- **Single direction chase** - LEDs illuminate sequentially from output 1 to 10
- **Low-power** - CMOS logic for efficient power consumption

## Specifications

- **Supply Voltage** -  3V to 15V
- **Logic Family** - CMOS
- **Power Consumption** - <50mA typical
- **Propagation Delay** - <100ms typical
- **Board Dimensions** - 70 x 70 mm

## Images

**Schematic**
<img src="images/dc-10_schematic.png">

**PCB Layout**
<img src="images/dc-10_pcb_layout.png">

**3D Model**
<img src="images/dc-10_pcb_model1.png">
<img src="images/dc-10_pcb_model2.png">

**Drawing**
<img src="images/dc-10_drawing1.png">
<img src="images/dc-10_drawing2.png">

# Previous Versions

## v1.0

This version had an issue with tracing and my inexperience with using Altium Designer. This led to the LEDs not lighting up sequentially in a clockwise fashion.

**Schematic**
<img src="images/dc-10_schematic.png">

**PCB Layout**
<img src="images/dc-10_pcb_layout.png">

**3D Model**
<img src="images/dc-10_pcb_model1.png">
<img src="images/dc-10_pcb_model2.png">

**Drawing**
<img src="images/dc-10_drawing1.png">
<img src="images/dc-10_drawing2.png">